...
- 적용된 UI 키트는 **"물과 빛"**(C안)이다. 근거와 팔레트 표는 `.design/hanaru-brand/DESIGN_BRIEF.md`.
  - 로고는 파스텔 톤으로 리컬러링해서 쓴다(원색 `#f41b26`/`#0b51db`는 더 이상 화면에 쓰지 않는다).
  - **명도 층을 지킨다**: 옅은 면(`--color-sea-pale`) → 흰 면(`--color-surface`) → 짙은 앵커(`--color-deep`). 파스텔만 나열하면 화면이 납작해진다.
  - 노을 계열(`--color-sunset*`, `--color-sun`)은 **본문 글자색으로 쓰지 않는다**(대비 3.3:1). 포인트 장식 전용.
  - 제목 서체(`font-display`, 고운돋움)는 **제목에만**. 내비·버튼·라벨·값은 Pretendard 굵기(600/700).
- 여백은 `--space-tight/snug/normal/loose/section` 스케일에서만 가져온다. 한 값을 반복하지 말고 좁게/넉넉하게를 의도적으로 대비시킨다. 섹션은 `.section` 유틸을 쓴다.
- 다크 모드는 쓰지 않는다 — 파스텔이 어두운 바탕에서 탁해지고, 주 방문자(어르신 포함)에게 밝은 화면이 더 편하다.
